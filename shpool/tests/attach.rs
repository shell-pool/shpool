...
fn prompt_prefix_bash() -> anyhow::Result<()> {
    ...
    waiter.wait_event("daemon-wrote-s2c-chunk")?;
    attach_proc.run_cmd("echo")?;
    lm.scan_until_re("hook_status=0 hook_ps=0$")?;
    lm.scan_until_re(".*session_name=sh1 prompt>.*")?;

    attach_proc.run_cmd("false | true")?;
    lm.scan_until_re("hook_status=0 hook_ps=1 0$")?;

    attach_proc.run_cmd("true | false")?;
    lm.scan_until_re("hook_status=1 hook_ps=0 1$")?;

    attach_proc.run_cmd("(exit 2) | (exit 3) | (exit 4)")?;
    lm.scan_until_re("hook_status=4 hook_ps=2 3 4$")?;

    attach_proc.run_cmd("echo")?;
    lm.scan_until_re(".*session_name=sh1 prompt>.*")?;

    // Verify that multiple prompt hooks each receive the original $? and
    // $PIPESTATUS
    attach_proc.run_cmd(
        r#"SHPOOL__OLD_PROMPT_COMMAND=('echo "h1 rc=$? ps=${PIPESTATUS[*]}"' 'echo "h2 rc=$? ps=${PIPESTATUS[*]}"')"#,
    )?;
    lm.scan_until_re("hook_status=0 hook_ps=0$")?;

    attach_proc.run_cmd("false | (exit 42)")?;
    lm.scan_until_re("h1 rc=42 ps=1 42$")?;
    lm.scan_until_re("h2 rc=42 ps=1 42$")?;
