...
// producing a duplicate event with two different timestamps. This test
// pins exactly one detached event per detach by using a kill as a
// known-next-event fence — if a duplicate detached were buffered, the
// next read would return it instead of `session.removed`.
#[test]
#[timeout(30000)]
