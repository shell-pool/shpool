...
pub trait SessionSpool {
    ...
    ///
    /// The returned sequence is expected to be able to restore the screen content regardless of
    /// any prior screen state. It thus mostly likely includes some terminal control codes
    /// to reset the screen from any state back to a known good state.
    ///
    /// Note that what exactly is restored is determined by the implementation, and thus can vary
    /// from do nothing to a few lines to a full screen, etc.
