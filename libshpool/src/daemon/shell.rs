            ...
            loop {
                ...
                if let Some(resize_cmd) = resize_cmd.as_ref() {
                    if resize_cmd.when.saturating_duration_since(time::Instant::now())
                        == time::Duration::ZERO
                    {
                        let status = pty_master.raw_fd().ok_or(anyhow!("no master fd"))
                    ...
                    (&mut client_conn, has_seen_prompt_sentinel)
                {
                    ...
                    let write_result = chunk.write_to(&mut conn.sink).and_then(|_| conn.sink.flush());
                ...
                loop {
                    ...
                    thread::sleep(consts::HEARTBEAT_DURATION);
                    {
                        ...
                        match shell_to_client_ctl
                            ...
                            Err(e) => return Err(e).context("requesting heartbeat from shell->client thread"),
                            _ => {}
                        }
