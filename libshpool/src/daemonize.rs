...
pub unsafe fn daemonize(pid_path: PathBuf) -> anyhow::Result<PidFileGuard> {
    ...
    // Safety: This is the only thread active because of the precondition,
    // which means the std file descriptors cannot be under contention right
    // now.
    unsafe { redirect_std_fds_to_null()? };

    Ok(PidFileGuard { p: pid_path })
}

// Safety: callers MUST NOT call this when the std file descriptors are
// in use.
unsafe fn redirect_std_fds_to_null() -> anyhow::Result<()> {
    let nullfd = fcntl::open(
        "/dev/null",
